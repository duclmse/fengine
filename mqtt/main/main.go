...
func main() {
	...
	logger.Info(fmt.Sprintf("Connect to mqtt subsriber: %s:%s\n", cfg.mqttHost, cfg.mqttPort))
	if err != nil {
		logger.Error(fmt.Sprintf("Failed to create MQTT subsriber: %s", err))
		os.Exit(1)
	}
	...
	if err := fwd.Forward(ms, w); err != nil {
		logger.Error(fmt.Sprintf("Failed to start MQTT forwarder %s", err))
		os.Exit(1)
	}
	...
	err = <-errs
	logger.Error(fmt.Sprintf("mqttv2 terminated: %s", err))
}
...
func connectToDevices(cfg config, logger mflog.Logger) *grpc.ClientConn {
	...
	logger.Info(fmt.Sprintf("Connect to devices url %s", cfg.devicesURL))
	conn, err := grpc.Dial(cfg.devicesURL, opts...)
	if err != nil {
		logger.Error(fmt.Sprintf("Failed to connect to devices service: %s", err))
		os.Exit(1)
	}
...
func connectToUsers(cfg config, logger logger.Logger) *grpc.ClientConn {
	...
	if err != nil {
		...
		logger.Info(fmt.Sprintf("Connected to users service by gRPC : %s", cfg.usersURL))
	}
...
func connectToOrg(cfg config, logger logger.Logger) *grpc.ClientConn {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Failed to connect to organization service: %s", err))
		os.Exit(1)
	} else {
		logger.Info(fmt.Sprintf("Connected to organization service by gRPC : %s", cfg.orgGrpcURL))
	}
